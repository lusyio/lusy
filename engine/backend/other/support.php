...
$supportPage = true;
$usersQuery = $pdo->prepare("SELECT DISTINCT u.id, u.idcompany AS companyId, u.name, u.surname, u.email, c.idcompany AS companyName, u.role AS userRole, IF(m.view_status = 0 AND m.recipient = 1, 0, 1) AS supportViewStatus FROM mail m LEFT JOIN users u ON (m.sender <> 1 AND m.sender = u.id OR m.recipient <> 1 AND m.recipient = u.id) LEFT JOIN company c ON u.idcompany = c.id WHERE m.sender = 1 OR m.recipient = 1 ORDER BY supportViewStatus, m.datetime DESC");
$usersQuery->execute();
...
foreach ($users as $user) {
    ...
    $messagesGroupedByCompany[$user['companyId']]['dialogs'][$user['id']]['role'] = $user['userRole'];
}

...
if (!empty($_COOKIE['token'])) {
    ...
    setcookie('token', createCookieString($sessionCookie['sid'], $sessionCookie['uid'], $timestamp), time() + 60 * 60 * 24 * 30, '/', $_SERVER['HTTP_HOST'], true);
    header('location: /');
    ob_end_flush();
    die();
} else {
        ...
        if (!empty($idc)) {
            ...
            if (!empty($id)) {
                ...
                if (password_verify($password, $hash) && !$isFired) {
                    ...
                    setcookie('token', createCookieString($sessionId, $id, $timestamp), time() + 60 * 60 * 24 * 30, '/', $_SERVER['HTTP_HOST'],true);
                    header('location: /');
                    ob_end_flush();
                    die();
                } else {

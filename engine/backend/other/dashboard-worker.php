...
$countTaskQuery = $pdo->prepare("SELECT COUNT(DISTINCT t.id) FROM tasks t LEFT JOIN task_coworkers tc ON t.id = tc.task_id WHERE t.status NOT IN ('done', 'canceled') AND (t.worker = :userId OR t.manager = :userId OR tc.worker_id = :userId)");
